...
- 正式予測モデル（lastKnownPos/confidence/predictedBehavior）は**未実装**（`TODO.md` §2-2）
...
| `computeVisionRadius()` @201 | 有視界半径（ヒッグス濃度連動） |
| `updateVisionLockOn()` @294 | 有視界ロックオン |
| `checkPassiveDetection()` @2826 | パッシブ探知（2秒毎、方位なし） |
| `fireOmniSonar()` @2869 | アクティブ/指向性ソナー（CD15秒） |
| `getHiggsIntensity(x,y)` | ヒッグス濃度（フレームキャッシュ付） |
| `buyUpgrade(type)` @4763 | アップグレード購入（engine/weapons/armor/sensor Lv0-3） |
| `generateSector()` @2528付近 | セクター生成 |
